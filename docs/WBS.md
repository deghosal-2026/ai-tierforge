...
- [x] **Description:** "Multi-model LLM tier router with cost-per-completed-task accounting. Route to the right model. Track the real cost."
- [x] **Topics:** `llm`, `cost-optimization`, `model-routing`, `python`, `open-source`
...
- [x] Tag `v0.1.0` on current `main`
- [ ] Publish to PyPI — needs trusted publisher config at https://pypi.org/manage/account/publishing/
- [x] GitHub Release created with description
- [ ] Verify `pip install ai-tierforge` works — depends on PyPI publish
...
**Done check:** Repo public, tag v0.1.0 pushed, release created. Pending: PyPI trusted publisher setup, article publish.
